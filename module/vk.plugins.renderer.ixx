...
export module vk.plugins.renderer;
import vk.context;
...
#define VK_CHECK(x) do { VkResult _vk_check_res = (x); if (_vk_check_res != VK_SUCCESS) { throw std::runtime_error(std::string("Vulkan error ") + std::to_string(_vk_check_res) + " at " #x); } } while (false)
    ...
    export class RendererTriangle {
    public:
        void query_required_device_caps(context::RendererCaps& c);
        void get_capabilities(context::RendererCaps& c);
        void initialize(const context::EngineContext& e, const context::RendererCaps& c, const context::FrameContext&);
        void destroy(const context::EngineContext& e, const context::RendererCaps&);
        void record_graphics(VkCommandBuffer cmd, const context::EngineContext&, const context::FrameContext& f);
    ...
    private:
        ...
        void initialize_device_state(const context::EngineContext& e, const context::RendererCaps& c) {
            dev = e.device;
        ...
        bool is_ready_to_render(const context::FrameContext& f) const {
            return pipe && !f.color_attachments.empty();
        }

        void prepare_for_rendering(VkCommandBuffer cmd, const context::AttachmentView& target) {
            transition_image_layout(cmd, target, VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);
        }

        void execute_rendering(VkCommandBuffer cmd, const context::AttachmentView& target, VkExtent2D extent) {
            begin_rendering(cmd, target, extent);
            draw_triangle(cmd, extent);
            end_rendering(cmd);
        }

        void finalize_rendering(VkCommandBuffer cmd, const context::AttachmentView& target) {
        ...
        void transition_image_layout(VkCommandBuffer cmd, const context::AttachmentView& target, VkImageLayout oldLayout, VkImageLayout newLayout) {
        ...
        VkImageMemoryBarrier2 create_image_barrier(const context::AttachmentView& target, VkImageLayout oldLayout, VkImageLayout newLayout, VkPipelineStageFlags2 srcStage, VkPipelineStageFlags2 dstStage, VkAccessFlags2 srcAccess, VkAccessFlags2 dstAccess) {
        ...
        void begin_rendering(VkCommandBuffer cmd, const context::AttachmentView& target, VkExtent2D extent) {
        ...
        VkRenderingAttachmentInfo create_color_attachment(const context::AttachmentView& target) {
